...
import { a4 as createServerFn, a5 as TSS_SERVER_FUNCTION, a6 as getServerFnById } from "./server-BoggyVwf.mjs";
...
const realSupabase = typeof window !== "undefined" && "https://rwgzletafpzfwwqtiwrg.supabase.co" && "sb_publishable_rToQ_5-F9Z0RzIC9g6XNew___KeCFPr" ? createClient("https://rwgzletafpzfwwqtiwrg.supabase.co", "sb_publishable_rToQ_5-F9Z0RzIC9g6XNew___KeCFPr") : null;
const supabase = {
  from(table) {
  ...
  channel(name) {
    if (realSupabase) {
      console.log(`[Realtime] Subscribing client to real Supabase channel: ${name}`);
      return realSupabase.channel(name);
    }
    console.warn(`[Realtime] Fallback to mock channel: ${name}`);
    return {
