import { c as createServerRpc } from "./createServerRpc-BQ-yBr-4.mjs";
import { a5 as createServerFn, a4 as getRequest } from "./server-D7ocmxQ4.mjs";
import { _ as __awaiter$1, a as __rest } from "./index-SkqIWSah.mjs";
import fs from "fs";
...
function runLocalQuery(query) {
  ...
  if (query.action === "insert") {
    ...
    for (const item of items) {
      const newItem = {
        id: item.id || crypto$1.randomUUID(),
...
const authSignIn = createServerFn({
  ...
  if (!supabase) {
    ...
    const sessionToken = crypto$1.randomUUID();
    const session = {
      access_token: sessionToken,
      token_type: "bearer",
      expires_in: 3600,
      refresh_token: crypto$1.randomUUID(),
      user: {
        id: user.id,
        email: user.email,
