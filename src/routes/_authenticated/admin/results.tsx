...
function ResultsPage() {
  ...
  const load = async () => {
    ...
    const pMap = Object.fromEntries(
      (p ?? []).map((x: any) => [x.id, formatDisplayName(x.display_name, x.username)]),
    );
    const qMap = Object.fromEntries((q ?? []).map((x: any) => [x.id, x.title]));
    setQuizzes(q ?? []);
    setRows(
      (a ?? []).map((r: any) => ({ ...r, username: pMap[r.user_id], quiz_title: qMap[r.quiz_id] })),
    );
  };
  useEffect(() => {
    load();
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl md:text-3xl font-bold">Results & Leaderboard</h1>
      </div>
      <p className="text-sm text-muted-foreground mt-1">
        Full visibility of every candidate's answers, including descriptive responses.
      </p>
                  ...
                  <td className="px-4 py-3 text-muted-foreground">
                    ...
                    {fmtDuration(r.started_at, r.submitted_at) && (
                      <div className="text-[10px] mt-0.5">
                        Took {fmtDuration(r.started_at, r.submitted_at)}
                      </div>
                    )}
                  </td>
            ...
            <div key={r.id} className="p-4 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                ...
                <div>
                  ...
                  <div className="font-semibold">{r.warnings || 0}</div>
                </div>
              </div>
              {r.submitted_at && (
...
function AttemptDetail({ attemptId, onClose }: { attemptId: string; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="bg-card rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-auto"
        ...
        <div className="sticky top-0 bg-card border-b border-border px-6 py-4 flex items-center justify-between z-10">
          <div>
            ...
            <div className="text-xs text-muted-foreground mt-1">
              Score {attempt.score} · {attempt.correct_count}/{attempt.total_questions} correct ·{" "}
              {attempt.warnings || 0} warning{(attempt.warnings || 0) === 1 ? "" : "s"} ·{" "}
              {attempt.status.replace("_", " ")}
            </div>
            <div className="text-xs text-muted-foreground mt-1 font-mono">
              Submitted: {fmtExact(attempt.submitted_at)}
              {fmtDuration(attempt.started_at, attempt.submitted_at) &&
                ` · Time taken: ${fmtDuration(attempt.started_at, attempt.submitted_at)}`}
            </div>
          </div>
          ...
          {rows.map(({ question: q, answer }, i) => {
            ...
            return (
              <div key={q.id} className="rounded-xl border border-border p-4">
                ...
                <div className="font-semibold mt-2 whitespace-pre-line">
                ...
                {isMcq ? (
                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    {sel ? (
                      <div
                        className={`rounded-lg p-3 text-sm font-mono border ${isCorrect ? "border-success bg-success/5 text-success font-semibold" : "border-destructive bg-destructive/5 text-destructive font-semibold"}`}
                      >
                        {sel}
                      </div>
                    ) : (
                      <div className="rounded-lg border border-dashed border-border p-3 text-sm text-muted-foreground italic">
                        No answer provided
                      </div>
                    )}
                    <div className="text-xs">
                      <span className="text-muted-foreground">Correct answer: </span>
                      <span className="font-mono bg-success/10 text-success px-2 py-0.5 rounded font-semibold">
                        {q.correct_answer}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-3">
                      ...
                      <div className="mt-3 flex items-end gap-2">
                        <div className="flex-1">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setGrading((g) => ({ ...g, [answer.id]: e.target.value }))
                            }
                          />
                        </div>
                    ...
                    {answer?.manual_score != null && (
                      <div className="mt-2 text-xs text-primary font-semibold">
                        Current grade: {answer.manual_score}
                      </div>
                    )}
                  </div>
                )}
