...
function ParticipantsPage() {
  ...
  const load = async () => {
    const [{ data: profiles }, { data: roles }] = await Promise.all([
      supabase
        .from("profiles")
        .select("id, username, display_name, access_code, member1_name, member2_name")
  ...
  const add = async () => {
    ...
    try {
      const res = await create({
        data: { member1_name: m1.trim(), member2_name: m2.trim(), access_code: code || undefined },
      });
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <p className="text-sm text-muted-foreground mt-1">
        Generate or customize access codes for exam takers. They sign in with the code only.
      </p>
        ...
        <div className="mt-5 rounded-2xl border-2 border-primary bg-primary/5 p-4 md:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[11px] uppercase tracking-widest text-primary font-semibold">
              New access code for {lastCode.name}
            </div>
            <div className="font-mono text-2xl md:text-3xl font-bold mt-1 tracking-widest break-all">
              {lastCode.code}
            </div>
          ...
          <Button variant="outline" onClick={() => copy(lastCode.code)} className="shrink-0">
          ...
          <div className="mt-3 space-y-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-1">
                A–Z and 0–9 only, 4–16 characters.
              </p>
            </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-muted-foreground">
                <tr>
                  ...
                  <th className="text-left px-4 py-2">Access code</th>
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-t border-border">
                    <td className="px-4 py-3 font-medium">
                      {formatDisplayName(r.display_name, r.username)}
                    </td>
                      ...
                      {editing?.id === r.id ? (
                        <div className="flex items-center gap-1">
                          <Input
                            value={editing.code}
                            onChange={(e) =>
                              setEditing({ id: r.id, code: e.target.value.toUpperCase() })
                            }
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => copy(r.access_code!)}
                            className="font-mono font-semibold inline-flex items-center gap-2 hover:text-primary"
                          >
                          ...
                          {r.role !== "admin" && (
                            <Button
                              size="icon"
                              variant="ghost"
                              onClick={() => setEditing({ id: r.id, code: r.access_code! })}
                            >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${r.role === "admin" ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}
                      >
                        {r.role}
                      </span>
                    </td>
              ...
              <div key={r.id} className="p-4 space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <span
                      className={`inline-block mt-1 text-[10px] px-2 py-0.5 rounded-full ${r.role === "admin" ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}
                    >
                      {r.role}
                    </span>
                  </div>
                ...
                {editing?.id === r.id ? (
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => copy(r.access_code!)}
                      className="font-mono font-semibold inline-flex items-center gap-2 hover:text-primary text-sm"
                    >
                      <KeyRound className="h-3 w-3" /> {r.access_code}
                    </button>
                    {r.role !== "admin" && (
