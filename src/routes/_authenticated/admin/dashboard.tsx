...
function LiveDashboard() {
  ...
  const isCameraOnline = (a: AttemptRow) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "cheat_events" }, () =>
        refresh(),
      )
      .subscribe();
      ...
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Realtime feed of every candidate's exam activity.
          </p>
        </div>
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
              {attempts.length === 0 && (
                <div className="col-span-full py-12 text-center text-muted-foreground text-sm">
                  No candidates active or registered yet.
                </div>
              )}
              {attempts.map((a) => (
                    ...
                    {attempts.map((a) => (
                      <tr key={a.id} className="border-t border-border">
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={
                              (a.warnings || 0) >= 2 ? "text-destructive font-semibold" : ""
                            }
                          >
                            {a.warnings || 0}
                          </span>
                  ...
                  <div key={a.id} className="p-4 space-y-2">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span>
                        ...
                        <b
                          className={
                            (a.warnings || 0) >= 2 ? "text-destructive" : "text-foreground"
                          }
                        >
                          {a.warnings || 0}
                        </b>
...
function Stat({
  ...
  return (
    <div className="rounded-2xl bg-card border border-border p-4 md:p-5">
      <div
        className={`flex items-center gap-2 text-[10px] md:text-xs font-medium uppercase tracking-widest ${colorClass}`}
      >
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ${map[status] ?? "bg-muted"}`}
    >
      {status.replace("_", " ")}
    </span>
  );
